...
NEXT: A0");
            ...
            Assert.NotEqual(maxTurnCount, result[0].TurnCount);

            Assert.True(result[0].Conversation.First().Messages.Last().Text.Contains("TERMINATE", StringComparison.OrdinalIgnoreCase));

            await CheckAnswer(result[0].Conversation.First().Messages.Last(), secretValues, "llama3:70b-instruct-q6_K");
        }
